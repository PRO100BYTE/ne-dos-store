...
app.get('/api/commands/:slug/source', async (req, res) => {
  const commands = await loadAllCommandsAndMeta();
  const command = commands.find((item) => item.slug === req.params.slug);
  if (!command) {
    res.status(404).json({ message: 'Command not found' });
  ...
  let filePath = null;
  if (command.origin === 'core') {
    const coreCommands = readJson(CORE_REGISTRY_FILE, []);
    const coreCmd = coreCommands.find((item) => item.slug === command.slug);
    if (coreCmd && coreCmd.packagePath) {
      filePath = path.join(__dirname, 'packages', 'core', coreCmd.packagePath);
    }
  } else if (command.origin === 'community') {
    filePath = path.join(COMMUNITY_DIR, `${command.slug}.js`);
  } else if (command.origin === 'submitted') {
    filePath = path.join(SUBMITTED_DIR, `${command.slug}.js`);
  }
  ...
  res.set('Content-Disposition', `attachment; filename="${command.slug}.js"`);
